...
/// Test double for URLProtocol used by URLSessionAPIClientSimulationTests.
/// Captures the most recent request + body and dispatches to a handler.
/// A dedicated class (not shared with the pricing protocol) so the two
/// serialized suites cannot clobber each other's captured request state.
final class SimulationMockURLProtocol: URLProtocol {
