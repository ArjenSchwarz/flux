...
The API base URL travels via `iCloudURLMirror` (`Flux/Packages/FluxCore/Sources/FluxCore/Settings/iCloudURLMirror.swift:18-72`). It is a `@MainActor` singleton (`shared` on line 20) but takes an injectable `KeyValueStore` protocol and `UserDefaults`/`NotificationCenter` in its public `init` (lines 30-38) so tests can substitute in-memory doubles. `NSUbiquitousKeyValueStore` is the cross-device source of truth; `start()` seeds the App Group `UserDefaults` from KVS, then observes `NSUbiquitousKeyValueStore.didChangeExternallyNotification` via an async-sequence loop on `MainActor` (lines 40-51). `pullFromRemote()` (lines 64-71) handles both the seed and the change-notification path — there used to be two methods, and the pre-push review collapsed them.
...
| [7.5](requirements.md#7.5) `tokenProvider` re-reads Keychain per request | Fully implemented | Existing `URLSessionAPIClient` behaviour preserved; widget runtime constructs the client via `WidgetRuntime.makeAPIClient` with a fresh `KeychainService` (`WidgetRuntime.swift:18-37`). |
