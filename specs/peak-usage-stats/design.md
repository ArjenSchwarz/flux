...
3. **Resolve `firstSolarTS` and `lastSolarTS` (Unix seconds).** When the corresponding tracker is nil, fall back to `computedSunrise.Unix()` / `computedSunset.Unix()` respectively. **Additionally**, when `solarStillUp` fires (per [decision 12](decision_log.md)), override `lastSolarTS` to `computedSunset.Unix()` and set `lastSolarFromFallback = true` even when the `lastSolar` tracker is non-nil. The override is needed because step 4's strict invariant uses `lastSolarTS`; without it, today + recent-solar requests during/before off-peak would fail the invariant and lose the morningPeak/offPeak detail. Record per-edge fallback flags (`firstSolarFromFallback`, `lastSolarFromFallback`) for use in step 9.
