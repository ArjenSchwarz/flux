...
- Day Notes backend (Lambda Go side): new `PUT /note` write endpoint (`{date, text}` JSON body) with the validation order from the spec — 415 on non-`application/json` content type, base64 body decode for Function URL clients that flag it, 413 for bodies over 4 KiB, 400 on malformed JSON / missing date / future date / >200 graphemes. Empty post-normalisation text deletes the note idempotently; non-empty text upserts. Handler routing in `internal/api/handler.go` now switches on method first (GET → status/history/day, PUT → note) and returns 405 with the correct `Allow` header for unknown method/path combinations. New `NoteWriter` interface in the api package (`PutNote`/`DeleteNote`) keeps the handler off `dynamo` internals; backed by `dynamo.DynamoNoteWriter` + a new `WriteAPI` interface (subset of `*dynamodb.Client`) so the IAM split between read- and write-side stays explicit at compile time. Note text is redacted from logs via `notePayload.LogValue` (only `date` and `text_len` ever land in slog). Shared text rules live in `internal/api/notetext.go`: `normalise` (Unicode NFC + leading/trailing-whitespace trim, internal whitespace preserved) and `graphemeCount` via `rivo/uniseg` — identical semantics to `NoteText.normalised`/`graphemeCount` on Swift, exercised against the shared `internal/api/testdata/note_lengths.json` fixture
- Day Notes read-side bundling: `Note *string` field added to `StatusResponse`, `DayEnergy` (per `/history` day), and `DayDetailResponse` — always serialised (`null` when absent). Each handler fetches its note(s) via a separate `sync.WaitGroup` goroutine outside the existing errgroup so a notes-table throttle logs and leaves the field `nil` instead of cancelling the sibling reads or returning 500. `/history` queries the full requested window (default 7 days) via the new `QueryNotes(serial, startDate, endDate)` reader method; `/status` and `/day` use the new `GetNote(serial, date)` point lookup. `dynamo.NoteItem` (`sysSn`, `date`, `text`, `updatedAt`) and `TableNames.Notes` added; both `Reader` and the new `WriteAPI` interfaces extended in lockstep with reader/writer tests
- `cmd/api/main.go` `config` gains a `notes api.NoteWriter` field threaded through to `api.NewHandler`; the wiring of `TABLE_NOTES` env var → `dynamo.NewDynamoNoteWriter` is left as a follow-up task so this commit covers the api/dynamo packages cleanly
- Cross-stack grapheme fixture at `internal/api/testdata/note_lengths.json` (Day Notes phase 1): eight named entries covering ASCII, NFD-decomposed `café` (e + U+0301), ZWJ family emoji, regional-indicator flag pair, skin-tone-modified emoji, trailing-whitespace trim, internal whitespace preservation, and a 200-character upper-bound case. Both the Go `notetext` tests and the Swift `NoteText` tests will load this exact file by repo-relative path so any divergence between `rivo/uniseg` and Foundation's grapheme counting fails on whichever side drifts. `specs/OVERVIEW.md` Day Notes status moved from `Planned` to `In Progress`
- Spec for "Day Notes" under `specs/day-notes/`: EARS requirements, design, 10-decision log, and 27-task implementation plan across two parallel streams (backend Go, iOS Swift) gated on a single shared cross-stack grapheme fixture. The feature adds the Lambda's first write endpoint (`PUT /note`, JSON body `{date, text}`) and a new `flux-notes` DynamoDB table (partition `sysSn`, sort `date`, PAY_PER_REQUEST, PITR enabled, `Retain` deletion policy) for at-most-one free-text note per calendar date per system. Notes are limited to 200 grapheme clusters after Unicode NFC + leading/trailing whitespace trim, with identical counting on both the Swift editor and the Go server enforced by `internal/api/testdata/note_lengths.json` consumed by tests on both sides; saving an empty note deletes idempotently (last-write-wins, no conflict detection). Notes bundle into the existing `/status` (today), `/history` (per-day), and `/day` read responses via a separate `sync.WaitGroup` goroutine outside the existing errgroup so a note-read failure logs and leaves the field `nil` without cancelling siblings. iOS renders a shared read-only `NoteRowView` between `BatteryHeroView` and `PowerTrioView` on Dashboard, between picker and chart cards on History (driven by the existing `selectedDay`), and as the first child of Day Detail (with tap-to-edit and an "Add note" affordance gated on date ≤ today Sydney). Editing happens only on Day Detail via a new `NoteEditorSheet`/`NoteEditorViewModel` mirroring the `SettingsView` sheet pattern; failed saves leave the editor open with text intact. The Lambda execution role gains `GetItem`/`Query`/`PutItem`/`DeleteItem` scoped to the new table only — existing read-only access on the other tables is unchanged. Note text is redacted from slog via `notePayload.LogValue`. `specs/OVERVIEW.md` updated with the new entry
- Household load (total usage) row in the energy summary views: Dashboard's "Today" card, History's per-day summary card, and Day Detail's summary card. Derived from the energy balance (`solar + grid_import + battery_discharge - grid_export - battery_charge`) via a shared `HouseholdLoad.kwh(...)` helper in `EnergySummaryFormatter.swift`; renders an em-dash whenever any of the five inputs is missing so partial data never produces a misleading total. Updated `EnergySummaryFormatterTests` to assert the new row and the energy-balance derivation
- Pre-push review for T-1021 peak usage stats: fixed `MockFluxAPIClient.dayDailyUsage` preview fixture timestamp ordering via a new `sydneyClockToUTC(date:)` helper; added Decision 12 to `specs/peak-usage-stats/decision_log.md` documenting the lastSolar override on `solarStillUp`; added `TestBuildDailyUsageBlock` (boundarySource matrix + elapsed<60s branch) and a "today + overcast mid-morning" `TestFindDailyUsage` fixture; wrote `specs/peak-usage-stats/implementation.md`
- Peak usage stats (T-1021): `/day` API now returns a `dailyUsage` object with up to five chronological no-overlap blocks (`night`, `morningPeak`, `offPeak`, `afternoonPeak`, `evening`) covering midnight to next midnight, each carrying `kind`, `start`/`end` (RFC 3339 UTC), `totalKwh`, optional `averageKwhPerHour`, integer `percentOfDay`, `status` (`complete`/`in-progress`), and `boundarySource` (`readings`/`estimated`). Powered by new `findDailyUsage` and `buildDailyUsageBlock` in `internal/api/compute.go` implementing the design's 11-step pipeline (single-pass `firstSolar`/`lastSolar` tracking inside a closed `[sunrise-30, sunset+30]` window for symmetric pre/post Ppv-blip filtering, solar-window invariant guard with two-block degradation, today-gate using a 5-minute recent-solar heuristic instead of astronomical sunset, future-omit, in-progress clamp, degenerate-omit, two-pass integration via `integratePload`). New `recentSolarThreshold = 5 * time.Minute` constant. iOS replaces `EveningNightCard` with `DailyUsageCard` rendering one row per block with positionally-adjacent `≈ sunrise`/`≈ sunset` captions when the relevant edge is estimated; an `(so far)` indicator marks in-progress blocks. Card slots into `DayDetailView` between `PeakUsageCard` and the summary card, gated on `viewModel.hasPowerData && !blocks.isEmpty`. New `BenchmarkFindDailyUsage` over 8640 readings stays in the same order of magnitude as the predecessor (~187µs/op)

 EARS requirements, design, 11-decision log, and 10-task implementation plan across two parallel streams (backend Go, iOS Swift). The feature replaces the existing Evening / Night card with a unified "Daily Usage" card that breaks each calendar date into up to five chronological no-overlap blocks (`night`, `morningPeak`, `offPeak`, `afternoonPeak`, `evening`), each carrying total kWh, average kWh/h, percent of day, status, and boundary source. The `eveningNight` field on `/day` is removed and replaced by `dailyUsage`; iOS clients update in lockstep. Off-peak boundaries come from the existing SSM parameters; solar boundaries reuse the Melbourne sunrise/sunset table with symmetric pre/post blip filters. Today-gate uses a recent-solar heuristic (any `Ppv > 0` reading in the last 5 minutes) instead of astronomical sunset, so cloudy late afternoons render the real evening block instead of waiting for sunset. Solar-window invariant violations and partial-data days where the recorder dies during off-peak collapse to the two-block path; only days where the recorder dies after off-peak end keep the five-block layout. `specs/OVERVIEW.md` updated with the new entry
- Pre-push review for T-1018 evening/night stats: typed status/boundary-source constants (`EveningNightStatusComplete`, `EveningNightStatusInProgress`, `EveningNightBoundaryReadings`, `EveningNightBoundaryEstimated`) in `internal/api/response.go`, replacing scattered string literals in `findEveningNight`, `compute_test.go`, and `day_test.go` (matches the existing `OffpeakStatus*` convention). `findEveningNight` now resolves the computed Melbourne sunset at most once per request via a small closure cache, removing a redundant lookup on the today-path. `MockFluxAPIClient.dayDetailResponse(for:)` was split into `dayReadings` and `dayEveningNight` helpers to clear a SwiftLint `function_body_length` violation introduced when extending the preview fixture. `APIModelsTests.swift` now also opts out of `file_length` lint (alongside the existing `type_body_length` opt-out) since the eveningNight decode coverage pushed it past the 400-line threshold. Added `specs/evening-night-stats/implementation.md` capturing the three-level implementation explanation and completeness assessment
- Evening / Night stats (T-1018): `/day` API now returns an `eveningNight` object with two optional `evening` / `night` blocks, each carrying `start`, `end` (RFC 3339 UTC), `totalKwh`, `averageKwhPerHour` (nullable when elapsed < 60 s), `status` (`complete` / `in-progress`), and `boundarySource` (`readings` / `estimated`). Boundaries come from the first/last `ppv > 0` reading on the requested date; missing readings fall back per-block to a Melbourne sunrise/sunset wall-clock value resolved through `time.ParseInLocation` in `sydneyTZ` (DST-immune). Today's evening block is gated on astronomical sunset to avoid emitting a 5-minute "evening" while the sun is still up; both blocks are clamped to `now` when in-progress. Powered by new pure functions `findEveningNight`, `integratePload` (half-open trapezoidal integration with negative-pload clamp before bracket interpolation and a 60 s pair-gap skip), `melbourneSunriseSunset`, and `buildEveningNightBlock`, plus a 365-entry `melbourneSunLocal` lookup table at `internal/api/melbourne_sun_table.go` (Feb 29 lookup falls through to Feb 28). Field is omitted on the daily-power fallback path and when no readings exist
...
- `eveningNight` field removed from `/day` response and the `EveningNightCard` removed from `DayDetailView`; both are superseded by the new `dailyUsage` field and `DailyUsageCard`. iOS clients update in lockstep; install-skew window during TestFlight rollout silently hides the card on stale builds
...
- Evening/Night stats (T-1033): `findEveningNight` no longer accepts a stray middle-of-the-night `Ppv > 0` reading as a block boundary. A sensor blip at e.g. 01:30 was ending the Night block at 01:30 in production, and on a day with no real production the same blip would also make the Evening block start at 01:30 (a 22+ hour "evening"). The `Ppv > 0` scan now ignores any reading whose timestamp is before `sunrise - 30 min` (Melbourne sunrise from the existing `melbourneSunLocal` table) for both the `firstPpv` (night-end) and `lastPpv` (evening-start) slots; when no qualifying reading exists, the block falls back to the table sunrise/sunset via the existing `boundarySource = estimated` path. No upper bound is applied — solar drops to zero well before sunset, so post-sunset blips are not a credible last-of-day reading and have not been observed.
