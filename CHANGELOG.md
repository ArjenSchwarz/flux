...
- **iPad adaptive layouts on Dashboard, History, and Day Detail** (T-1150). At regular horizontal size class on iPad, the three main screens now reflow into multi-column arrangements via the `AdaptiveColumnsLayout` helper. Dashboard puts the battery hero and live trio side by side and reflows the Summary / Battery / future blocks through 2-col (at ≥ 700pt) or 3-col (at ≥ 1000pt) grids. History keeps the full-width stats overview card on top, then reflows the Solar / Grid Usage / Daily Usage / selected-day summary cards through the same grid. Day Detail switches to a two-column Grid with summary panels (Day-in-Five-Blocks, Summary, Battery, Daily Usage, Peak Usage) on the left and three stacked chart panels (Power, Battery Power, State of Charge) on the right, with the day navigation header, note section, and Compare control full width above. The Settings sheet on iPad regular is capped to a 640pt-wide column centred in the sheet so form rows aren't stretched edge-to-edge. iPhone and macOS layouts are unchanged.
...
- **iPad adaptive layout — AppNavigationView wiring** (T-1150). Hoists Dashboard, History, and the Today `DayDetailViewModel` instances into `AppNavigationView` so they survive an iPad size-class flip between shells; `FluxiOSRoot` accepts them via init rather than constructing internally. Adds a `today` state, a 60 s recompute task, and a `scenePhase → .active` recompute that together call `DayDetailViewModel.setDate(_:)` on the hoisted Today VM whenever the local date rolls over. Rebuilds the three hoisted VMs in `reloadDependencies()` when the API client identity changes so a credentials swap doesn't leave them with a stale (unauthorized) client. Adds bidirectional `selectedScreen` ↔ `iosTab` sync via two `onChange` handlers that guard against feedback loops by comparing before writing. Still no user-visible change on iPhone or macOS — the iPad shell is added in a later phase.
