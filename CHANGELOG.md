...
- **SoC Alerts — per-device APNs environment** (T-1288). The first cut of the feature loaded `/flux/apns/env` from SSM once at poller startup and used that host for every push. With one user on Xcode dev builds (sandbox tokens) and the other on TestFlight (production tokens) on the same backend, whichever environment didn't match the SSM value would silently 400 every push and end up with `tokenStatus = "stale"`. The poller now holds two `apns2.Client` instances (same `.p8` key, different hosts) and routes each push against the environment carried on the device row. The Swift app reads its own `aps-environment` entitlement at runtime via `SecTaskCopyValueForEntitlement` and sends it in registration. The Lambda accepts and persists the new `apnsEnvironment` field, validates it (must be `"development"` or `"production"`), and preserves the existing value on token-less re-registrations. The `/flux/apns/env` SSM param and `APNS_ENV_PARAM` env var are gone — only four APNs params left to set up. An unrelated `JSONDecoder` defect surfaced by the tests is also fixed: `URLSessionAPIClient` now uses `.iso8601` for date decoding/encoding so rule timestamps from the Go backend round-trip correctly (without this, every `GET /devices/{id}/rules` response would have failed to decode in production).
