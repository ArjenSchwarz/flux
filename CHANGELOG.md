...
- **Solar by Block — backend integration core** (T-1162). New `derivedstats.integratePpv` returns trapezoidal kWh and reading-sample count over a half-open window, mirroring `integratePload`'s 60s pair-gap rule, edge synthesis, and `max(ppv, 0)` clamping. `derivedstats.Blocks()` now accumulates a `SolarKwh *float64` on the `morningPeak`, `offPeak`, and `afternoonPeak` blocks; the sample count distinguishes "no readings in window" (emits `nil`) from "samples present but integral is zero" (emits `&0.0`). Night and evening blocks remain `nil`. Includes table-driven `integratePpv` tests, a `pgregory.net/rapid` split-additivity property test, and `Blocks()` cases covering sunny / winter / reading-gap / in-progress today / no-readings / heavy-fog / single-reading scenarios.
- **Solar by Block — DynamoDB persistence** (T-1162). `dynamo.DailyUsageBlockAttr` gains a `SolarKwh *float64` field tagged `dynamodbav:"solarKwh,omitempty"`, carried through `DailyUsageFromAttr`/`DailyUsageToAttr` so the new value rides the existing atomic `UpdateDailyEnergyDerived` write path. `&0.0` round-trips as non-nil (preserving the "samples present, integral zero" signal), `nil` is omitted from the stored item. Round-trip unit + `pgregory.net/rapid` property tests cover both states; the post-feature sizing fixture now populates `SolarKwh` on all three daylight blocks and still serialises under the 4 KB single-RCU boundary (~2.4 KB).
- **Solar by Block — backfill CLI** (T-1162). New `cmd/backfill-solar` standalone Go CLI walks `flux-daily-energy` rows in a configurable `--from`/`--to` Sydney-TZ date range (defaulting to the 30-day readings TTL window), recomputes `solarKwh` for daylight blocks via `derivedstats.Blocks()` against the still-available readings, and patches `solarKwh` field-by-field onto the existing stored blocks. `totalKwh`, `start`, `end`, `boundarySource`, `percentOfDay`, `status`, and `averageKwhPerHour` are preserved byte-for-byte (Decision 7) so partially TTL-pruned readings cannot drift historical totals. `--dry-run` logs intended writes without invoking `UpdateItem`; rows whose daylight blocks all already carry `solarKwh` are skipped (Decision 4 idempotency). Tests cover dry-run, live single-write, idempotency, byte-for-byte preservation, and the missing-row / missing-readings / query-error paths.
- **Solar by Block — API and poller test coverage** (T-1162). `/day` past-date and live today paths assert `solarKwh` is present on `morningPeak`/`offPeak`/`afternoonPeak` and absent on `night`/`evening`; the today fixture uses tight reading pairs so `integratePpv` produces non-zero values during the in-progress live-compute branch. `/history` fixtures (`history_bench_test.go`, `history_derivedstats_test.go`) populate `SolarKwh` on daylight blocks. Poller summarisation gains `TestSummarisation_PersistsSolarKwhOnDaylightBlocks`, asserting the captured `UpdateDailyEnergyDerived` payload carries `SolarKwh` on each daylight block. The DynamoDB Local e2e test now asserts `SolarKwh` survives MarshalMap → real read on the stored row and matches across the `/day` and `/history` API responses.
- **Solar by Block — `dailyUsage` write-path invariant** (T-1162). Doc comment on `dynamo.UpdateDailyEnergyDerived` records that it is the only writer of the `dailyUsage` attribute outside the `cmd/backfill-solar` CLI, so future writers cannot be added without revisiting the backfill's idempotency assumptions.
- **Solar by Block — pre-push polish** (T-1162). `cmd/backfill-solar` `validateOpts` now rejects reversed `--from`/`--to` ranges with a clear error so an operator typo can no longer silently backfill zero rows. `DayInFiveBlocksPanel` time, total-kWh, and solar-kWh `Text` views gain `lineLimit(1)` + `minimumScaleFactor(0.85)` so the now-four-value daylight rows shrink instead of wrapping/clipping at narrower widths and larger Dynamic Type sizes (Decision 3 negative consequence). New `TestValidateOpts_RejectsReversedDateRange` covers the CLI guard. `specs/solar-by-block/implementation.md` records the implementation at three explanation levels and a per-AC completeness assessment.
- **Solar by Block — iOS Day Detail rendering** (T-1162). `FluxCore.DailyUsageBlock` gains an optional `solarKwh: Double?` decoded from the `/day` and `/history` payloads (present, zero, null, and absent JSON forms all decode unambiguously thanks to a default-`nil` initializer parameter, leaving every existing call site source-compatible). `DayInFiveBlocksPanel` appends an amber `sun.max.fill` glyph and a matching kWh value next to the existing usage figure on the morning peak, off-peak, and afternoon peak rows when the API supplies a value; null is rendered as a clean omission per AC 3.4 so days predating the feature deploy do not show false zeros. Night and evening rows are untouched.
