...
- **Time-of-use pricing** (T-1890, T-1891). An electricity plan is now a set of daily time bands rather than three flat rates, so a plan with different prices at different times of day can be described directly. A plan is entered as a default rate plus the windows that deviate from it — for example free 10:00–15:00 and a cheaper rate 01:00–06:00, with the default applying the rest of the day. Times outside every window take the default, so a plan cannot be entered with a gap.
  - **Settings ▸ Pricing** lists plans with their date range and bands — "Free 10:00–15:00 · $0.2800 01:00–06:00 · $0.3500 default" — and the editor gains a Windows section with per-window start and end pickers, a Free toggle, and a rate field. Validation in the app mirrors the server's rules, so a plan the editor accepts is not rejected afterwards for something it could have caught.
  - **Plans hand over on a date.** You can enter a new plan ahead of time and it takes over automatically on its start date. Ending a plan on the date its successor starts gives that whole day to the successor, and the editor's affordance now says exactly that rather than "the day before".
  - **The free window comes from the plan**, not from server configuration. It moves with the plan on the switch date, with nothing to reconfigure — including the Day Detail chart shading, the off-peak card, charge projection, and cutoff suppression, which on the eve of a switch already follow the incoming plan's window. A day whose plan has no free window shows no window rather than a default one.
  - **Costs are computed from the bands**, resolving to the stored per-band split where it is available and falling back to the previous calculation otherwise. Every historical day's cost is unchanged: the migration verified all 101 priced days as identical before writing anything. Cost figures agree between Day Detail and History, online or from cache.

### Internal

- The per-band energy split is captured when each day closes, so banded costs survive past the 30-day raw-readings retention. Each stored split and off-peak row records the window geometry it was measured under, so a later plan edit is detectable rather than silently repricing history.
- The poller reads plans directly and resolves each day's window from them, serving a last-good cache if the pricing table is briefly unreachable — a failed read is never treated as "no plan". The off-peak scheduler is re-anchored to local midnight, and the summarisation pass distinguishes a transient read failure from a day with no free window and from a day no plan prices, rather than collapsing all three into one early return.
- `cmd/migrate-pricing` converted the existing plan once, verifying every retained day's costs under an independently written copy of the previous formula before writing. It has run in production; the transitional read-side conversion it gated is removed, and the legacy shape is now rejected rather than repaired. Both backfill CLIs resolve each day's window per day, so a backfill spanning a switch date attributes each side correctly.
- `OFFPEAK_START`/`OFFPEAK_END` and the matching CloudFormation parameters are gone. The poller container gains read-only access to the pricing table; the Lambda keeps sole write access. Deploying this version leaves `/flux/offpeak-start` and `/flux/offpeak-end` behind as orphaned SSM parameters that nothing reads.
- Band boundaries resolve on the day's wall clock, which also retires a latent hour-off bug on the two daylight-saving transition days a year. Property-based tests assert the bands always tile the full day and that per-band energy sums to the whole-day total on 23-, 24-, and 25-hour days.

### Documentation

- Spec for the above: Time-of-use pricing (`specs/time-of-use-pricing/`), covering both tickets, with the cutover record and its operational gotchas.
- **Architecture diagrams** (`docs/architecture-diagrams.md`). Eight Mermaid diagrams covering the system overview, build and deploy pipeline, AWS infrastructure, the polling engine, the DynamoDB data model, the Lambda API surface, and the dashboard and push sequences — each drawn from the source of truth and validated against the Mermaid parser.
- Corrected the DynamoDB table list in `CLAUDE.md` to enumerate all 11 tables with their retention policy.
