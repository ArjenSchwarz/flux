...
- **Stat Comparisons — Day Detail wiring** (T-1161, phase 4, completes T-1161). `DayDetailView` hosts `CompareControl` directly above the five-block panel and forwards `viewModel.comparisonState` into both `DayInFiveBlocksPanel` and the Day Detail `SummaryBlock`. Toggle and period preferences persist per-device via `@AppStorage` against new `compare.enabled` / `compare.period` keys in the `fluxAppGroup` suite, with `ComparePeriod.parseOrDefault` bridging the rawValue store. Three `.onChange` reactions — `compareEnabled` (with `initial: true` so a previously-on toggle re-fires the fetch on appearance), `comparePeriodRaw`, and `viewModel.date` — drive `viewModel.updateCompare(...)` so the comparison reissues whenever the toggle, chip, or selected day moves.
