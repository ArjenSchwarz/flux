...
- Pre-push review for T-1021 peak usage stats: fixed `MockFluxAPIClient.dayDailyUsage` preview fixture, which produced impossible UTC timestamps (morningPeak end before start) by appending Sydney clock times to a single date string — replaced with a `sydneyClockToUTC(date:)` helper that converts Sydney-local (hour, minute) pairs to proper RFC 3339 UTC across midnight boundaries. Added Decision 12 to `specs/peak-usage-stats/decision_log.md` documenting the lastSolar override on `solarStillUp` (an undocumented behaviour the implementation needed to keep the five-block path viable on today + recent-solar mid-day requests); cross-referenced from `design.md` step 3 and `compute.go` inline comment. Added `TestBuildDailyUsageBlock` table test covering the boundarySource matrix (both readings, start estimated only, end estimated only, both estimated) and the `elapsed < 60s` `AverageKwhPerHour` omission branch that was uncovered by `TestFindDailyUsage`. Added the "today + overcast mid-morning, no qualifying Ppv yet" fixture (originally listed in `tasks.md` task 2 but missing from tests) — exercises Decision 12's override path and asserts `morningPeak in-progress` with `boundarySource = "estimated"`. Wrote `specs/peak-usage-stats/implementation.md` with three-level explanation and per-AC completeness assessment
