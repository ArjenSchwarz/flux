...
- Evening/Night stats (T-1033): `findEveningNight` no longer accepts a stray middle-of-the-night `Ppv > 0` reading as the night-end boundary. A sensor blip at e.g. 01:30 was ending the Night block at 01:30 in production. The first-`Ppv > 0` search now ignores any reading whose timestamp is before `sunrise - 30 min` (Melbourne sunrise from the existing `melbourneSunLocal` table); when no qualifying reading exists, the block falls back to the table sunrise via the existing `boundarySource = estimated` path. Evening side intentionally untouched — solar production stops well before sunset, so the symmetric noise problem hasn't been observed.
