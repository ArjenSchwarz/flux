...
- `.github/workflows/deploy.yml` — deploys the Lambda API and forces an ECS poller redeploy on push to `main`. The `deploy-api` job runs `go test`, builds the ARM64 `bootstrap` binary, then runs `aws cloudformation package` + `aws cloudformation deploy` (passing only `ContainerImageUri`; existing stack parameters are inherited). The `update-containers` job is gated on a `workflow_run` completion of "Poller CI" (so the new `:latest` is in GHCR before ECS pulls it) and runs `aws ecs update-service --force-new-deployment` plus `aws ecs wait services-stable`. Auth uses GitHub OIDC via `secrets.AWS_DEPLOY_ROLE_ARN`; `secrets.CFN_PACKAGE_BUCKET` holds the S3 bucket used by `cloudformation package`; region defaults to `ap-southeast-2` and can be overridden with repo variable `AWS_REGION`. `workflow_dispatch` is wired up for manual runs of either job.
