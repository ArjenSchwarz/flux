...
- Medium widget: the SOC readout now sits inside a circular progress ring (new `SOCRing` view) tinted with the battery-colour tier — Apple-battery-widget style. The charge/discharge status line (e.g. "Discharging at 2.10 kW · cutoff ~17:12") moved from under the SOC to under the power-trio stats on the right, so the left column is a single focused battery-state element. Small, large, and accessory widgets unchanged.
